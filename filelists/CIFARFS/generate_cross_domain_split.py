...
def write_json(class_list, name, class_name_to_path, class_name_to_label):
    ...
    with open(join(base_path, f"{name}.json"), 'w') as f:
        json.dump(d, f)
...
def generate_cross_domain_split(base_classes, val_classes, novel_classes):
    ...
    :return:
    """
    with open(join(base_path, "class_name_to_label.pickle"), 'rb') as f:
        class_name_to_label = pickle.load(f)
    with open(join(base_path, "class_name_to_path.pickle"), 'rb') as f:
